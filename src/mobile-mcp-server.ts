...
async function showMobileConfirmation(args: {
  ...
  const resolvedDeviceId = await resolveDeviceId(args.deviceId);
  return await postJson<Record<string, unknown>>(
    `/api/users/${encodeURIComponent(config.userId)}/agents/${encodeURIComponent(config.agentId)}/mobile-agents/${encodeURIComponent(resolvedDeviceId)}/show-confirmation`,
    {
      title: args.title,
      text: args.text,
...
function extractConfirmations(events: MobileLogEvent[]): MobileLogEvent[] {
